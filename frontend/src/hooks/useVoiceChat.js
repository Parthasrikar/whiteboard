...
export const useVoiceChat = (socket, roomCode, userName) => {
  ...
  const initializeAudioStream = useCallback(async () => {
    try {
      ...
      stream.getAudioTracks().forEach(track => {
        track.enabled = false;
        console.log('🎵 Audio track initialized:', track.label, 'enabled:', track.enabled);
      });
  ...
  const updatePeerStatus = useCallback((userId, updates) => {
    setConnectedPeers(prev => {
      ...
      console.log(`👥 Updated peer ${userId} status:`, { ...currentStatus, ...updates });
      return newPeers;
    });
  }, []);
  ...
  const createPeerConnection = useCallback((userId) => {
    ...
    if (localStreamRef.current) {
      localStreamRef.current.getTracks().forEach(track => {
        console.log(`➕ Adding track to peer connection for ${userId}:`, track.kind, track.enabled);
    ...
    peerConnection.ondatachannel = (event) => {
      console.log(`📡 Data channel received from ${userId}:`, event.channel.label);
    };
  ...
  const playRemoteAudio = useCallback((userId, stream) => {
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch(error => {
          console.error(`❌ Error playing remote audio for ${userId}:`, error);
  ...
  const initiateVoiceChatWithUsers = useCallback(async (userIds) => {
    if (!localStreamRef.current || isInitiatingRef.current) {
      console.log('⚠️ Cannot initiate voice chat - no local stream or already initiating');
      return;
    }
  ...
  const toggleMute = useCallback(() => {
    ...
    audioTracks.forEach(track => {
      ...
      console.log(`🎵 Audio track ${newMutedState ? 'muted' : 'unmuted'}:`, track.label);
    });
    ...
    if (socket) {
      ...
      console.log(`📡 Sent mute status to server: ${newMutedState ? 'muted' : 'unmuted'}`);
    }
  }, [isMuted, socket]);
  ...
  if (peerConnection) {
    ...
    const handleVoiceIceCandidate = async (data) => {
      ...
      if (peerConnection && peerConnection.remoteDescription) {
        ...
        console.warn(`⚠️ Cannot add ICE candidate from ${fromUserId} - no peer connection or remote description`);
      }
    };
    ...
    const handleVoiceChatResponse = async (data) => {
      ...
      console.log(`📞 Voice chat response from ${fromUserId}: ${accepted ? 'accepted' : 'rejected'}`);
    ...
    const handleUserVoiceStatus = (data) => {
      ...
      console.log(`🎵 User ${userId} voice status: ${isMuted ? 'muted' : 'unmuted'}`);
  ...
  }, [socket, createPeerConnection, initializeAudioStream, initiateVoiceChatWithUsers, updatePeerStatus]);
